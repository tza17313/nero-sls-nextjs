  ...
  async deploy(inputs) {
    ...
    if (inputs.staticConf) {
      ...
      regionList.forEach((region) => {
        const staticDeployer = async () => {
          const res = await this.deployStatic(credentials, inputs.staticConf, region)
          staticOutputs[region] = res
        }
        staticTasks.push(staticDeployer())
      })
      ...
      if (regionList.length === 1) {
        const [oneRegion] = regionList
        this.state.staticConf = staticOutputs[oneRegion]
        outputs.staticConf = staticOutputs[oneRegion]
      } else {
        this.state.staticConf = staticOutputs
        outputs.staticConf = staticOutputs
      }
    }
